...
async fn get_meetings_from_dbus(
    ...
    for source_uid in source_uids {
        ...
        // Query from 1 week ago to 4 weeks in the future
        let now = Local::now();
        let query_start = now - chrono::Duration::days(7);
        let query_end = now + chrono::Duration::days(28);
...
/// Returns true if the meeting starts within the query window (from query_start onward).
...
mod tests {
    ...
    #[test]
    fn test_should_include_meeting_past_within_window() {
        let now = Local::now();
        let query_start = now - chrono::Duration::days(7);
        let start = now - chrono::Duration::hours(2);
        let end = now - chrono::Duration::hours(1);

        // Past meeting within query window - should be included
        assert!(should_include_meeting(start, end, now, query_start));
    }

    #[test]
    fn test_should_include_meeting_past_before_window() {
        let now = Local::now();
        let query_start = now - chrono::Duration::minutes(30);
        let start = now - chrono::Duration::hours(2);
        let end = now - chrono::Duration::hours(1);

        // Past meeting before query window - should not be included
        assert!(!should_include_meeting(start, end, now, query_start));
    }
    ...
    #[test]
    fn test_should_include_meeting_started_before_query_window() {
        let now = Local::now();
        let query_start = now - chrono::Duration::minutes(30);
        let start = now - chrono::Duration::hours(1); // Started 1 hour ago (before query window)
    ...
    fn test_should_include_meeting_just_ended() {
        ...
        // Meeting just ended but within query window - should be included
        assert!(should_include_meeting(start, end, now, query_start));
    }
